...
| `assignment_mode` | `String(32)` | **Not directly editable** — written by the assignment-generation path that ran, and reset to NULL when every assignment is deleted. The rule-based engine is the only path that writes it, and `rule_based` is the only value the `AssignmentMode` enum admits; `manual` is a legacy cell value no code path writes. |
...
| `display_timezone` | `String(64)` | IANA zone name used to render this session's dates / times. Resolution order: session value → creating operator's default → UTC. NULL means "inherit the operator default" and is only reachable on rows written before the forms always stamped a zone — Create and Edit both write a concrete one, so a new session is never NULL; the middle resolution step exists for those older rows. Set on the Create Session form's **Timezone** field and the Edit Session Details form's **Timezone** field — both pre-filled, both also scoping the deadline picker (the deadline is wall-clock in this zone). Every session-scoped operator + reviewer surface renders dates / times in the resolved zone. |
