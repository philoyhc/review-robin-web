...
**Reachability is asked per role, in `build_role_chips`, and never
inherited from the route's own gate.** Roster membership is not an
answer to it: a chip built from membership alone renders a live
Reviewee chip for a caller whose `/results` answers 404, because the
two conditions are different ones. A caller on `/collation` has passed
the observer gate and nothing else, so the reviewer and reviewee
questions are re-asked here on every surface that includes the strip.
