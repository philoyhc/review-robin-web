...
- Observer collation: reachable for any active observer, on any session but an **archived** one — `build_observer_collation_context` short-circuits there and returns no sections, which is why the `/me` link is unlinked in that state. The per-instrument visibility-policy resolver inside `build_observer_collation_context` applies the window gate at view time — instruments only render when the active window (while_ongoing / after_release) has a policy-permitted mode for the observer audience. The route itself does not 403 based on the release window.
...
- **Currently, not ever-configured.** A policy row whose window has not opened does not count. Otherwise an operator's setup-time decision would announce the review to its subject months before they may see anything, which is the disclosure this rule closes.
