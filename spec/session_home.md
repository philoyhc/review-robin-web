...
| **Draft, pre-generate** — `is_draft`, rosters populated, every instrument has its rule pinned, AND either no `Assignment` rows yet OR the session was reverted to draft (Revert / Pause) after the last generation | "Run generation to create the assignment pairs (note that doing so will replace any previously generated assignment pairs)." | **Generate assignments** (POST `/assignments/generate`) | — *(none)* |
