...
| `color_tokens.md` | Colour-token inventory — every `base.html` `:root` colour token (friendly name, light + dark value, where used), grouped by family, plus the non-colour scale tokens. The catalogue for the palette `visual_style_rrw.md` describes semantically. |
