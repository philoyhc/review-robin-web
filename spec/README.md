...
| `participant_model.md` | The cross-cutting participant-model contract — three rosters (reviewer / reviewee / observer), per-session feature toggles, the `/me` dashboard cross-role union and priority-pick linking, `/me/sessions/{id}/results` + `/collation` placeholder surfaces, schedule columns (`responses_release_at` / `release_until_offset`), and the access-control invariants pinning operator vs participant separation. Single entry point — the per-page detail lives in `setup_pages.md`, `reviewer-surface.md`, `role_navigator.md`. |
