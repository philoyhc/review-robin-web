...
| 161 | Operator actions (15F) | Delete | `<button type="button">` | `btn destructive` | Destructive | Deletes the checkbox-selected rows via `/reviewers/bulk-delete`. Sits between `Add` and `Search`. Two-stage gate (Segment 19I): a selection enables the `Yes, delete these` checkbox on the status row, which enables this button through the §7 pairing (`data-delete-btn="reviewers-bulk-delete"`). **Inert as of PR 1** — a `type="button"` no-op with no route behind it; the route lands in PR 3. |
...
| 162 | Operator actions (15F) | Delete | `<button type="button">` | `btn destructive` | Destructive | Deletes the checkbox-selected rows via `/reviewees/bulk-delete`. Sits between `Add` and `Search`. Two-stage gate (Segment 19I): a selection enables the `Yes, delete these` checkbox on the status row, which enables this button through the §7 pairing (`data-delete-btn="reviewees-bulk-delete"`). **Inert as of PR 1** — a `type="button"` no-op with no route behind it; the route lands in PR 3. |
...
| 163 | Operator actions (15F) | Delete | `<button type="button">` | `btn destructive` | Destructive | Deletes the checkbox-selected rows via `/relationships/bulk-delete`. Sits between `Add` and `Search`. Two-stage gate (Segment 19I): a selection enables the `Yes, delete these` checkbox on the status row, which enables this button through the §7 pairing (`data-delete-btn="relationships-bulk-delete"`). **Inert as of PR 1** — a `type="button"` no-op with no route behind it; the route lands in PR 3. |
