...
| # | Card | Label | Element | CSS class | Canonical | Notes |
|---|---|---|---|---|---|---|
| 105 | Reviewer tag labels | Cancel | `<button type="button">` | `btn secondary` | Secondary | Inline JS reverts the three tag inputs to their initial snapshot and re-disables the pair. Suppressed whenever the session is not `is_editable`. |
| 106 | Reviewer tag labels | Save labels | `<button type="submit">` | `btn secondary` | Secondary | Posts `/reviewers/field-labels`. Starts `disabled`; inline JS flips both Save + Cancel on when the form is dirty. Suppressed whenever the session is not `is_editable`. |
...
| 127 | Operator actions | Clear | `<a>` | `btn secondary` | Secondary | Resets the filter; rendered only when a filter is active. |
| 128 | Operator actions (Edit/Add) | Save | `<button type="submit">` | `btn primary` | Primary | Submits the `/{id}/update` or `/create` form; shown below the divider in Edit/Add mode. |
| 129 | Operator actions (Edit/Add) | Cancel | `<a>` | `btn secondary` | Secondary | Returns to the plain list. |
...
| 135 | Operator actions (Edit/Add) | Save | `<button type="submit">` | `btn primary` | Primary | Submits the `/{id}/update` or `/create` form; shown below the divider in Edit/Add mode. |
...
| 142 | Operator actions (Edit/Add) | Save | `<button type="submit">` | `btn primary` | Primary | Submits the `/{id}/update` or `/create` form; reviewer / reviewee chosen via name-or-email `<datalist>` pickers. |
