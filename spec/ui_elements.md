...
fill the row. **The answer since is to drop the grid**, not to pin the
survivor — a grid with one child is not a grid, and a plain card is
page width without any rule at all. That is what 19P.5 rung 3 did on
Invitations and Responses, where the survivor was the info card;
`spec/operations_pages.md` § *Shared page shape* records why full
width beat half width flush right for a counters card.
