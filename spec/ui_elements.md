...
| `rf-table` | Band 3's response-field table (`spec/instruments.md` § *Response fields*, 19T Item 9). One `<tbody>` per field, ruled underneath (`border-bottom: 1px solid var(--border-default)` on the `<tbody>`) and not inside it (`border-bottom: 0` on each `<td>`), so a governed field can later join its parent's `<tbody>` as one ruled group. The name and type columns take fixed shares (20% / 15%); the bounds column takes the rest. An empty name box (`td.rf-name input`) shows its default label muted, as its placeholder (`color: var(--text-subtle)`, `opacity: 1` — a placeholder is `0.7`-ish by default in most browsers, which this overrides so the default reads as clearly as a typed name would) |
| `.btn-short` | A shorter `.btn` for a compact table row — same role and outline, 19px tall inside a 32px row. First user: the display-field table's ▲ ▼ (`spec/instruments.md` § *Display-field table*, 19T Item 9, on the author's ruling); the response-field table's ▲ ▼ stay full-size |
