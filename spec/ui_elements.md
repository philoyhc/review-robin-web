...
| **Landing targets** — `#<noun>-table-card`, `tr.row-action-target`, `#<noun>-row-editor` / `#<noun>-row-<id>` | **Three targets since 19P.1, and only the first is a card.** A page turn, and on Reviewers a filter or a Search / Clear, lands on the **table card** (`scroll-margin-top: var(--space-4)` — enough to keep the top border off the viewport edge so it reads as a boundary rather than a crop). A **row action** lands on the row it acted on, via `tr.row-action-target` at **`scroll-margin-top: 88px`** — deliberately larger than the cards' 16px, because a row flush against the viewport edge reads as the table's first row rather than as one row among others, and because the row's own expander renders *below* it: 88px clears one 68px row plus 16px, putting the acted-on row second from the top with a neighbour visible for context. Nothing on these pages is `position: sticky`, so there is no fixed chrome to land underneath. **Entering edit mode takes the third**: `#<noun>-row-editor` is an **add-mode-only id on the `<tr>` itself**, and `Edit` builds `#<noun>-row-<id>` from the id it already has — there is no editor card in the contract, the row is the editor. A fourth anchor, `.roster-card`, exists for the same reason as the first: a labels save returns to `#roster-card` so the Unlock panel stays open, and without a `scroll-margin-top` it arrived cropped. **A fragment that does not resolve is ignored by the browser and lands at the top of the document**, so any surface using a row fragment must also ship the fallback that catches a missing target — three cases reach it: a deleted row, a row the active filter excludes, and a row moved by a cookie-held sort |
...
| `.table-card-toolbar` (+ `.is-split`, `.toolbar-pane`, `.toolbar-left`, `.toolbar-right`) | **Two bare panes at the head of a table card** (19P.1, Reviewers). Card geometry — the same half-and-half split and gutter `.card-columns` gives — with **no border, fill or padding of its own**, because these are regions of one card rather than two cards. That is the distinction §10 already draws against `.card-columns`, now with a name. Left pane: what the table is showing (column chips, pager cluster, count line). Right pane: the filter strip that decides it. The strip moved here from a card a grid away, so the controls sit with the rows they act on |
