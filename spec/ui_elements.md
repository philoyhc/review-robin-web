...
| `.session-row-selected` — a selected row on the sessions lobby. **Edge and fill**: `--selected-bg` as a `box-shadow: inset 3px 0 0` on the first cell, `--row-selected-bg` as the row's background. The pair `.tag-chip.is-selected` already uses, in row shape — the reserved shade for the edge, its pale companion for the interior — because a selected row *is* the thing the panel's actions will act on, which is what that shade means. An inset shadow rather than a border, so selection does not change the row's height and reflow the table under the pointer. **Lobby-local for now**: named here so it is findable, not promoted to a general primitive, because it has exactly one caller and designing for a second (the Rosters idea in `guide/new_ux_ideas.md`) before it exists is how an abstraction acquires the wrong shape. **Not scanned by `tests/unit/test_reserved_shade.py`, and not exempted from it**: that guard's filter is pill / chip / `btn-icon` classes, because its subject is elements with a dual nature — a `<tr>` has one. Applied in `refreshExpander()`, the single funnel every selection path meets, clearing all rows each pass before marking the selected set | added 2026-09-12 | — |
