...
| `.table-pager` (+ `.table-pager-bottom`, `.table-pager-link`, `.table-pager-gap`, `.table-pager-jump`) — the row pager on the seven roster-bearing tables, rendered above the table and again below it. Ranges (`201–400`), not page numbers, with First / Last hung off the ends and `…` marking each elided gap. Suppressed whenever a search or status filter is active. **Every range is an ordinary link** — the page's own `a` colour and the user agent's underline, no padding, radius or `text-decoration: none`; the strip's `gap` does the spacing. The current page is a `<span aria-current="page">` marked by weight alone, never by a fill: it is the one cell here that does not navigate, and the accent shade `--blue-strong` / `--blue-glow` is reserved for things that do. Each range link carries a `#<noun>-table-card` fragment so a page turn arrives at the **table's card**: its top edge, then the column chips, then the page links, then the new rows. The id sits on the card, with a `scroll-margin-top` so the card's top border reads as a boundary rather than a crop; the route supplies the id, the pager never derives it | added 2026-09-11 | — |
