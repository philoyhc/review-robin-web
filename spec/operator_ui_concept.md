...
1. Session top nav.
2. Yellow lock card when `ready` (with `return_to=reviewers` / `reviewees` / `relationships` so the operator returns here after reverting). Sits immediately under the info card — same status-info-then-yellow-lock pattern the Instruments page uses. **Not Assignments**: P4 above already records that the three post-Operations pages retired their `.card.lock` notices, and this line contradicted it (corrected in Segment 19I Item 8).
3. **Friendly-label editor (left) + Operator actions card (right)** — a half-width `bottom-grid` pair. The friendly-label editor is the inline editor for the per-session tag-column labels; the Operator actions card (Segment 15F) carries the search / status filter strip and the selection-driven Edit · Inactivate · Activate · Add-new-row button row. See `spec/setup_pages.md` "Operator actions card".
