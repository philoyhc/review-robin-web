...
Fields: Name (required, max 255), Code (required, max 64; unique per operator), Timezone (required, IANA-zone `<datalist>`, pre-filled with the operator's default), Deadline (optional, datetime-local — interpreted as wall-clock in the picked Timezone), Description (optional, max 2000), Help contact (optional). The Schedule sub-grid also surfaces **Release responses from** (`responses_release_at`) and **Release responses until** (`responses_release_until`, an absolute close `datetime-local`, never a duration). All four `datetime-local` inputs (Start / End / Release-from / Release-until) carry `min` / `max` attributes the browser picker honours — Start ≤ End ≤ Release-from ≤ Release-until — and a small shared partial (`operator/partials/_schedule_ordering_js.html`) live-updates the bounds as the operator types. The server re-runs `scheduled_events.validate_schedule_ordering` as the load-bearing safety net (see `spec/lifecycle.md` §8.2.7). A **User interface settings** card above the Quick Setup card carries two checkboxes: **Enable Relationships tab** (`relationships_enabled`) and **Enable Observers tab** (`observers_enabled`); both default unchecked. Heading the right column, a **Tags (optional)** card carries one comma-separated text box (19S Item 6) that completes the tag at the end of the line from the operator's own tags as it is typed (19S Item 7, the shared tag typeahead described in `spec/sessions_overview.md`). Below the Session details card the page is a `.bottom-grid` of two `.bottom-left` columns — User interface settings over Quick Setup on the left, Tags over Owners on the right — so each column's `gap` spaces its cards (cards inside `.bottom-grid` carry no `margin-bottom`), and the placements approximate Session Home's (author's ruling, 2026-09-23). Its `<h3>` is the field's visible label (`aria-labelledby`), so there is no second "Tags (optional)" above the box. Like the UI toggles, it associates with the create form via `form="create-session-form"` and submits with **Create session**; there is no second button. Under Tags, an **Owners (optional)** card (19S Item 9 Part A) — its shape, staging and how it compares with Session Home's own Owners card are in `spec/session_owners.md`. Action row: **Create session** (Primary) submits to `POST /operator/sessions` → resolves the staged owners first (an address that is not a workspace operator is a 422 and creates nothing), then inserts the session + a `SessionOperator` row + a `session.created` audit event, then any tag writes, then a `session.owner_added` event per co-owner, all under one correlation id + 303 to `/operator/sessions/{id}`. **Cancel** (Secondary) → `/operator/sessions`.
