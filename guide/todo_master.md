...
3. **Segment 11G — Validate page.** The chrome's Operations-row Validate tab (`/operator/sessions/{id}/validate`) shipped on v2 during 11A but as a thin read-only issue list. 11G makes the page work for "find and fix": readiness summary card + setup-coverage matrix + per-issue "Fix on {Setup page} ↗" deep-links + severity filter chips + per-rule "Why this check?" disclosure + activate-warns detour banner that moves the warning-acknowledgment ceremony off the cramped Next Action card on Home. Sized as 4 PRs. **Plan: `guide/segment_11G_validate_page.md`.**
4. **Segment 11H — Placeholder card scaffolds (Quick Setup + Extract Data).** Replaces the two `placeholder_card(...)` stubs on Session Home with their inert-but-fully-rendered real shapes — every slot / row / button visible and laid out, but every interactive control disabled until 11J / 12A wire them up. Pins the visual + DOM contract (Jinja partials, macros, `QuickSetupSlot` / `ExtractDataRow` dataclasses, fragment anchors, lifecycle / lock-card behaviour) so the wiring PRs that follow are thin diffs. Sized as 2 PRs (parallelizable; touch disjoint placeholder sites). **Plan: `guide/segment_11H_placeholder_card_scaffolds.md`.**
5. **Segment 11J — Quick Setup.** Wires the four-slot Quick Setup card 11H scaffolds. Each PR flips one slot from `is_wired=False` to `True` and supplies a `wire_url` against the existing per-entity import pipeline; no markup is created in this segment. Three live slots (Reviewers / Reviewees / Assignments) plus a fourth Configuration-import placeholder that graduates when Segment 12A PR 6 lands. Sized as 3 PRs (PR C may collapse to a no-op once 11H ships). Catalog `unfinished_business.md` #30. **Plan: `guide/segment_11J_quick_setup_card.md`.** (Letter 11I is skipped to avoid I/1 ambiguity, a common convention.)
6. **Segment 11K — #5 Audit-event `detail` schema convention.** Spec write-up in `spec/architecture.md` (canonical envelopes — `changes` / `snapshot` / `counts` / `set_changes` — plus orthogonal identity / `reason` / `refs` slots) + typed audit helpers in `app/services/audit.py` + incremental emitter migration (one PR per family) + optional Pydantic write-validation gate. Sized as ~8 PRs. The last sub-segment before Segment 12; pinning the `detail` shape now means Segment 12's audit-export consumer reads a stable schema. Catalog `unfinished_business.md` #5. **Plan: `guide/segment_11K_audit_event_detail_schema.md`.**
...
- **Items 2, 3** polish or build out specific Operations-row tabs (Previews, Validate). They share patterns with 11C (list-with-bulk-actions, the chrome Operations row) and benefit from 11C landing first. 11G PR D's activate-detour banner depends on the assumptions.md banner-Cancel convention 11J pinned; ship after 11J's PR A.
- **Item 4 (11H)** scaffolds the two Session Home placeholder cards (Quick Setup + Extract Data). It's a visual-only segment — no service-layer code, no routes, no audit events — so the card layout, mobile collapse, lifecycle states, and DOM IDs / fragment anchors all settle in one set of small PRs before any wiring debate. Required prerequisite for item 5 (11J) and 12A PR 6. (11H was previously listed for "Extract Data" specifically, then retired when that work folded into 12A; this item is a re-purpose under the same letter, scaffolding both placeholder cards.)
- **Item 5 (11J)** wires Quick Setup against 11H's scaffold. Each PR is a thin diff that flips one slot live; no markup creation.
- **Item 6 (11K)** gates Segment 12 (audit retention). Last sub-segment before Segment 12 starts.
