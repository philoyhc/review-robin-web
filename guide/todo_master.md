...
4. **Segment 11J — Quick Setup.** The Quick Setup card on Session Home ships today as a placeholder via the canonical `placeholder_card` macro (Segment 11B). 11J builds out the real card per `spec/quick_setup_card_spec.md` — three live slots (Reviewers / Reviewees / Assignments) plus a fourth Configuration-import placeholder that graduates when Segment 12A's import route lands. Sized as 3 PRs. Catalog `unfinished_business.md` #30. **Plan: `guide/segment_11J.md`.** (Letter 11I is skipped to avoid I/1 ambiguity, a common convention.)
...
(Previously item 4 — **Segment 11H — Extract Data** — has been folded into **Segment 12A** (`guide/segment_12A.md`), which now ships the Extract Data card on Session Home as PR 6 alongside the configuration round-trip work. The standalone 11H entry is retired.)
...
- **Item 4 (11J)** is the only Home placeholder-card build-out remaining (Extract Data has folded into 12A). Reuses the v2 vocabulary (`.card.placeholder` retires for the Quick Setup card once 11J's real implementation lands).
