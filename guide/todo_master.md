...
6. **Segment 11H — Responses page.** New reviewee-centric Operations page (`session_responses.html`, `/operator/sessions/{id}/responses`) per `spec/operations_renew.md` "Responses page". Lists reviewees with coverage status (Complete / Adequate / At risk / No responses), filtering, selection, bulk reminder dispatch, per-row drill-in. Plan TBD. **Overlaps with Segment 11C item 3** — 11C as currently planned bundles Invitations rewrite + Responses page + Monitoring retirement; if 11H is to be its own segment, 11C's scope narrows to "Invitations rewrite + Monitoring retirement" and 11H takes the new Responses page on its own. Sequencing TBD; flag for clarification before either starts.
7. **Segment 11J — Extract Data.** The Extract Data card on Session Home ships today as a placeholder via the canonical `placeholder_card` macro (Segment 11B). 11J builds out the real extraction surface — picks which formats to support (CSV / JSON / etc.), wires the response dump endpoint, and ships the format selector + counts summary inside the card. Likely overlaps with the original Segment 12 (export / audit retention MVP) — see "Deferred" section. Plan TBD.
8. **Segment 11J — Quick Setup.** The Quick Setup card on Session Home ships today as a placeholder via the canonical `placeholder_card` macro (Segment 11B). 11J builds out the real card per `spec/quick_setup_card_spec.md` — bulk-populate reviewers, reviewees, and assignments from files or rules in one place. Catalog `unfinished_business.md` #30. Plan TBD. **Note: shares the "Segment 11J" label with item 7 (Extract Data) per the user's list — verify whether they're meant to bundle as one segment or are two segments with one needing a different letter (perhaps 11I, often skipped to avoid I/1 ambiguity).**
9. **Segment 11K — #5 Audit-event `detail` schema convention.** Spec write-up in `spec/architecture.md` then incremental emitter migration. Segment 12 (export / audit retention) reads `audit_events` and needs the `detail` JSON shape pinned first; this is the latest item that gates Segment 12. Catalog `unfinished_business.md` #5. Plan TBD.

### Notes on the order

- **Items 1, 2, 3 are roughly disjoint surfaces** (non-session chrome / `/setupinvite` / Operations row) and can ship in parallel if capacity allows. The user's listed order (D → E → C) suggests 11D first, then 11E, then 11C — likely reflecting which surface most needs settling next.
- **Items 4, 5, 6** all polish or build out specific Operations-row tabs (Previews, Validate, Responses). They share patterns with 11C (list-with-bulk-actions, the chrome Operations row) and benefit from 11C landing first.
- **Items 7, 8** build out the two Session Home placeholder cards. They depend on 11D having no special prerequisite but reuse the v2 vocabulary (`.card.placeholder` retires for these specific cards once their real implementation lands).
- **Item 9** gates Segment 12 (export / audit retention). Should be the last P2-tier item before Segment 12 starts. If Extract Data (item 7) is folded into Segment 12 instead of a separate 11J, the sequencing becomes 11K → Segment 12.

### Pending clarifications

- **11H vs 11C**: 11C as currently planned (`guide/segment_11C.md`) bundles Invitations rewrite + Responses page. 11H would split Responses out. Decide before either starts.
- **Two "11J"s**: items 7 (Extract Data) and 8 (Quick Setup) share the segment label. Likely typo (skipping 11I to avoid letter / digit ambiguity is a common convention; one of these probably wants a different letter). Clarify before plans land.
- **Segment 12 scope**: with Extract Data potentially carved off as 11J and audit-event schema as 11K, Segment 12 (`guide/segment_12_export_audit_retention_mvp_plan.md`) narrows to "audit retention" only — or 11J subsumes it. Reconcile when 11J's plan drafts.
