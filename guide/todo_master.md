...
- **Segment 11F Part 1 — Previews page (chrome + picker + invitation email + reviewer surface)** — done 2026-05-06. The Operations-row Previews tab (`/operator/sessions/{id}/previews`) graduates from a placeholder to the pre-flight Reviewer Experience Preview hub. Three of the five planned PRs landed:
  ...
  - Plan: `guide/segment_11F_previews_page.md`. **PR E shipped via Segment 11E PR 6 (#532)** — the responses-received tab activation rode along with the editor third tab, since both depend on the same `render_responses_received` helper + the EMAIL_PREVIEW_TABS registry mutation. **Part 2 collapses to PR D only** — wiring the Reminder tab to `email_templates.render_reminder`.
...
3. **Segment 11F Part 2 — Previews page (Reminder tab).** Part 1 (PRs A / B / C) shipped; PR E (Responses-received tab) shipped via Segment 11E PR 6 (the responses-received tab rode along with the editor third tab on the same `render_responses_received` helper + EMAIL_PREVIEW_TABS registry mutation). **Collapses to PR D only** — wiring the Reminder entry to `email_templates.render_reminder(session, reviewer)`; same `EmailBody` shape as the invitation card. **Plan: `guide/segment_11F_previews_page.md`** ("PR D" subsection).
...
- **Item 3 (11F PR D)** is a thin diff against the existing `EMAIL_PREVIEW_TABS` registry — same shape as the responses-received tab activation that already shipped via 11E PR 6. Independent of Items 1 / 2.
