...
- **#1838, #1839** — Two addendum updates to the Codex weaknesses doc (`guide/archive/weaknesses_and_bugs_found_by_codex.md` post-retire) reflecting ship status as Slices A/B/C, then the Codex P1 follow-up, landed.

**Deferred as Slice D** (own future slice): storage-level uniqueness guard — a lower-expression unique index, or normalize-on-write. Gated on the lookup + tolerance fixes baking on the dev slot plus a read-only duplicate-cluster audit against production data; the addendum carries the open question.

**Slices E + F set aside** (no scheduled work): large templates (`instruments_index.html` 5,342 LOC, `base.html` 3,231 LOC, `session_extract_data.html` 2,610 LOC) and other stale-comment sweeps. Decision rationale + flip-conditions captured in the addendum — opportunistic only when a functional PR is already touching the relevant file.
...
**Docs-sweep follow-through — ✅ all four buckets done 2026-08-19** (PRs
#1977 + follow-on); `docs/` went 20 → 16 files:

- **Revise into `spec/`** — ✅ `docs/rehydrate.md` → `spec/rehydrate.md`,
  trimming the build-scaffolding sections now the feature has shipped.
- **Update in place** — ✅ status.md header refreshed to 18S + 18Q row + VNet
  reword + retired `/edit` rows; the cross-cutting "no in-app revoke UI (16A
  PR 6)" claim fixed in three docs (shipped 18S); the conftest / test-count
  inversion fixed in two docs; participant gates added to security_posture;
  + the per-file factual fixes.
- **Consolidate** — ✅ cli_setup_notes → cli_setup; authentication →
  security_posture (identity subsystem + full CSRF write-up absorbed);
  codespace_setup → local_setup §9; azure_github_setup reconciled in place
  with a "future PRD/NPRD scale-up" banner (kept in docs/).
- **Retire** — ✅ deleted imports.md + cli_setup_notes.md; section-level
...
- **Item 2 (✅ shipped 2026-08-21, W1–W8)** — **light / dark
  ...
  `_partials/theme_toggle.html` in the operator chrome + reviewer top bar —
  reaches participants, who can't see `/operator/settings`); the scaffolded
