...
### Segment 12A-1 — Session export (settings + per-entity CSVs) — done 2026-05-09 (PRs #713, #716, #717, #718)

Splits the umbrella "Segment 12A — Session settings import + export" into the export half (this segment) and the import half (12A-2, see Upcoming below). Ships four CSV downloads off the Extract Data card on Session Home: Settings (PR 1) + an audit-log fallback that captures the seeded RuleSet a rule-based session was generated against (PR 1a) + Reviewers / Reviewees (PR 2) + Manual Assignments (PR 3). Plan: `guide/segment_12A-1_export.md`.
...
Out of scope (deferred): responses extract, zip bundle, and the import side (12A-2 — `guide/segment_12A-2_import.md`).
