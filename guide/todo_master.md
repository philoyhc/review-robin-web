...
- **Segment 11E — Operator-editable email template editor + SMTP scaffolding** — done 2026-05-05. Five PRs landed against the plan (PR 3 collapsed into PR 1 — the renderer wiring landed there) plus one polish PR:
  ...
  - Plan: `guide/segment_11E_email_template_editor.md` (un-archived 2026-05-05 to host a "Responses-received email" follow-on section — **1 unshipped PR (PR 6)** adding the `email_template_overrides` schema additions, `render_responses_received` helper, editor third tab, and the per-session "Send when reviewer submits?" checkbox. The submit-time send wiring formerly planned as 11E PR 7 is **absorbed into Segment 11C Part 2 PR H** — 11C now owns all email *sending* and 11E retains only authoring. After PR 6 ships, 11E retires and the plan archives). Catalog `unfinished_business.md` #24 (closed by this segment).
...
1. **Segment 11C Part 2 — Email send activation.** Wires Segment 11E's `EmailTransport` interface into the Manage Invitations page Part 1 just rebuilt, and absorbs the responses-received submit-time enqueue (formerly Segment 11E PR 7). Three PRs:
   ...
   - **PR H** — reviewer-submit handler enqueues a responses-received outbox row when the per-session toggle (`email_template_overrides.responses_received_enabled`, default `True`) is on; new `responses_received_email.queued` audit event. Per-reviewer-per-session, not per-assignment. Hard prerequisite: **Segment 11E PR 6** must ship first (it introduces the toggle key + the `render_responses_received` helper); PRs F + G are unaffected and can land before 11E PR 6.
   Hard prerequisites already met for PRs F + G: Part 1 PR C ships the consolidated page; Segment 11E PRs 4 + 5 ship the operator Settings page + transport interface. **Plan: `guide/segment_11C_operations_consolidation.md` "Part 2".**
...
- **Item 1 (11C Part 2)** ships the email send-activation work that consumes 11E's transport interface. The load-bearing piece for getting real emails out the door. **PR H is gated on Segment 11E PR 6** (the responses-received editor follow-on); the other two are unblocked.
