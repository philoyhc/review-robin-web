...
**58 of 85 resolved. 27 open.**

| what the open rows need | ids | count |
|---|---|---|
| **a ruling** — which side is right | `SC-13`…`SC-15`, `SC-17`, `SC-19`…`SC-21`, `SC-23`, `SC-24`, `SC-26`…`SC-28`, `SC-30`, `SC-31`, `SC-33`…`SC-36` | 18 |
...
| `SC-19` | `role_navigator.md` | identity match is `func.lower(column) == casefold(email)` in SQL | `build_role_chips` folds via `normalize_email` in Python | **(c)** — *re-bucketed from (a) by the author, 2026-09-13, for later investigation.* The two sides fold case differently (SQL `func.lower` vs Python `normalize_email`), and whether they agree on every input is the question — `normalize_email` may do more than lower-casing, in which case this is a behaviour difference and not a documentation one |
