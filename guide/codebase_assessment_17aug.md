...
All shipped 2026-06-03 → 2026-08-18 (104 merge commits, 129 non-merge, over
~76 calendar days — a markedly lower cadence than the prior window's 79
merges in two days). Numbers taken on `main` at `5a5ccbc`. Development context
unchanged: single author + AI coding agents, pre-deployment, no production
traffic yet.

A standalone snapshot; `guide/codebase_assessment_03jun.md` archives alongside
it. Authoritative ship-state lives in `docs/status.md`; functional specs
audited against live in `spec/`.
...
|---|---|---|---|
| `production` (`app/**/*.py`) | 192 (185 prior) | **55,136** | +2,218 (+4.2%) |
| `app/web/templates/` | 60 (59) | **21,771** | +301 (+1.4%) |
| `tests/` | 251 (240) | **86,743** | +3,254 (+3.9%) |
| `docs` (`docs/` + `spec/` + `guide/` + top-level `*.md`) | 200 (180) | **88,312** | +6,587 (+8.1%) |
| Alembic migrations | 77 (76) | **6,772** | +69 (+1.0%) |

**Test-to-production-Python ratio ~1.57×** (86,743 / 55,136), flat vs ~1.58×
on 03jun. **2,647 tests passing, 17 skipped** (was 2,546 + 17; +101 over the
...
**Where the window's growth landed.** Production grew +2,218 LOC but only
**+7 files** — almost all *new* production code is the 18P rehydrate feature
...
existing guards. This is the most diagnostic figure in §2: a 102-PR window that
moved production LOC only +4.2% because two of its three big arcs were
*re-shaping*, not *adding*.

**Docs outgrew code** (+6,587 doc LOC vs +2,218 production) — the `guide/`
...
| **Three-tier role model (operator ⊂ admin ⊂ super-admin)** | `spec/audience_and_identity_model.md` §4 | **✓ shipped (18S, 2026-08-17)** |
...
- **Two of three big arcs were re-shapes that shipped without a migration.** 18R
  ...
  green across the entire 102-PR window; `ruff` clean at the pin.
...
- **18S admin-management footgun, documented but unresolved.** With no
  `SUPER_ADMIN_EMAILS` set (it's optional), *no one* can promote/demote admins —
  the actor-super guard has no valid actor. Localhost is covered by
  `fake_auth_super_admin`; a real deploy must set the list. Flagged in the #1926
  PR body and the plan; no fallback shipped. *Cost:* a mis-provisioned deploy
  could lock its own admin management.
- **Docs outgrew code and the `guide/` corpus is accreting.** +6,587 doc LOC
  this window (vs +2,218 production); several `guide/segment_*.md` plans now
...
No known open bugs at `5a5ccbc`. Basis: full suite green (2,647 passed, 17
...
Today: **55,136 production Python + 21,771 templates + 86,743 tests** (+ 6,772
...
we're already past it at 55,136 — because **18P rehydrate (~+1.3k) was
...
spent this window **re-shaping more than adding**: 102 PRs moved production LOC
...
LOC on the back of the consolidated `/save`. The nearest real risk is
operational, not architectural: the 18S no-`SUPER_ADMIN_EMAILS` footgun.

**Recommended next moves** (≤3):

1. **Resolve the 18S admin-management footgun before any deploy that sets no
   super-admin** — either require `SUPER_ADMIN_EMAILS` in the fail-fast, or add
   the "plain admins retain admin-management when the super tier is
   unconfigured" fallback. It's next because it's a lockout hazard on the exact
   surface 18S just shipped, and it's a small, well-scoped change.
2. **Start Segment 14B email infrastructure** — it's the largest remaining MVP
   scope, unblocks the W20 invite triggers, and is the thing standing between
   the current state and a pilot that can actually notify participants.
3. **Run a `guide/` archive sweep** — move the fully-shipped 18P/18R/18S plan
   docs to `guide/archive/` (as 18O did) so the plan corpus stops out-growing
   the code it describes. Cheap, and it keeps the next assessment's §5 honest.
