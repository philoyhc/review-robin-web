...
| `extract_data.md` | Plan to split today's Session Home **Extract data** card into a slimmer **Extract setup** card (porting-shaped CSVs only) and a new **Extract data** Operations tab whose job is fine-grained shaping of response data for offline analysis (by instrument / by reviewer / by reviewee lenses). Independent of the queued schedule. |
