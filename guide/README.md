...
| `self_review_consolidate.md` | Plan to consolidate every self-review classification path onto a single `Assignment.is_self_review` column, written at Assignment-creation time via the canonical whole-group rule and consumed by every extract / counter. Fixes a latent bug in the By-instrument CSV (hardcoded `SelfReview = FALSE` on group-scoped rows) along the way. Sequenced **ahead of** the *Self-review handling* chip slice in `extract_data.md` so the chip can read the column from day 1. Sized at 5 small PRs. |
