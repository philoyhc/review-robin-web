...
| `observers.md` | Standing notes for the observer participant role — what's shipped on the operator side and the wiring tail (W5 / W17) still to ship. **Paused 2026-06-01** while observer use scenarios are rethought. Inherited the open observer-side items from the retired participant-model docs (now archived). |
| `clean_up.md` | Standing punch-list of small-to-medium follow-ups flagged by code review but not yet shipped. Pick from the top in idle moments; each item carries its own context + source flag. |
