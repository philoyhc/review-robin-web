...
| `segment_plan_template.md` | Blank segment / item plan with every section prompted — Opportunity → Decision → Semantics → Judgment calls → Blast radius (measured) → PR ladder → Definition of done → Open questions → Out of scope → Doc impact (+ a Status block added when intended and done diverge). Two headings are machine-read by the planned `tools/close_check.py` (`## Doc impact`, `## Status`); do not rename them. The operational guide for using it is the `segment-plan` skill (`.claude/skills/segment-plan/SKILL.md`). |
