...
| `theme_customizer.md` | Idea / not-scheduled plan for a **theme customizer**, as two composable plans sharing one editor core: **First** = a developer *designer* in the `tools/` harness (seed-and-derive OKLCH editing → JSON a coding agent ports into `base.html`); **Stretch** = an operator *tweaker* in the app that saves **browser-local** (localStorage + runtime-apply, no migration). DB-backed shared/persistent themes noted as a further future neither plan does. No commitment; recorded so it's ready to pick up. |
