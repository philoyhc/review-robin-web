...
| `ux_theme.md` | Working sweep + code punch-list for the light / dark **Display mode** (Segment 19C Item 2) — browser-local `data-theme`, the base.html token / raw-hex inventory, non-base light-islands, and the W1–W8 work items. Retire to `archive/` once dark mode ships. |
| `theme_preview.html` + `theme_preview.gen.py` | In-repo dark-mode design harness for `ux_theme.md` W4 — a standalone HTML mockup (open in a browser; no server) that lifts base.html's real CSS, applies a draft dark palette, and renders a component gallery + 47-token swatch grid with a live System / Light / Dark toggle. `.gen.py` regenerates the HTML from base.html. Design tool only; retire with `ux_theme.md`. |
