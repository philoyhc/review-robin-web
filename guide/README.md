...
| `sweep_<YYYY-MM-DD>_<scope>.md` | A dated sweep's working notes. The filename's date is machine-read by `tools/close_check.py --stale`; keep the shape. **The newest one stays here until a newer sweep supersedes it** — `last_sweep_date()` globs `guide/` non-recursively, so archiving it does not error, it silently turns the staleness trigger off: `--stale` goes from "not due, 3 d of 56, 100 of 500 merges" to "No dated sweep found" and exits 0 either way. Archive the older ones freely (2026-09-08). |
...
| `inplace_pagination_assessment.md` | **Assessment, not a plan** — should the roster tables turn pages by swapping the table in place rather than reloading? Written 2026-09-11 at `88f8849d`, because 19J.8 rejected the idea on the phrase "several times the work" and that is not a number anyone can decide from. Measures the page turn (413KB, the table 47% of it; 21ms at 556 rows, 116ms at 5,000), what a DOM swap would break (of five table-relevant script blocks in `base.html`, **one** is delegated — the other four, ~500 lines, bind to elements and die with them), and the precondition (928 lines of table markup inline across seven templates, none in a partial). Recommends **not yet**, names three cheaper options with what each buys, and one fix worth doing either way. **Settled 2026-09-11 (`Decision` section): not worth solving** — the only jarring case is a click on the bottom strip, and an operator who turns a page and stays there means to read the new range from its start anyway. Read it as the support for that decision, not as an open question; the delegation fix is unaffected. Carries its own reproduction notes; re-take the numbers rather than trusting them later. |
