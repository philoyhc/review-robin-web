...
| `theme_customizer.md` | Idea / not-scheduled plan for growing the `tools/` theme-preview harness into a **theme customizer** — seed-and-derive (OKLCH) colour editing with live repaint + JSON export. Phase 1 (harness-only, migration-free) vs Phase 2 (app consumption, needs a migration). No commitment; recorded so it's ready to pick up. |
