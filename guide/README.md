...
| `participant_model_prep.md` | Implementation-phase audit pairing with `participant_model_upgrade.md`: every schema / placeholder / wiring item classified Schema → Placeholder → Wiring, with the full ship trail inline. Authoritative for what shipped + how. |
| `participant_model_remainder.md` | Focused filter on `_prep.md` — outstanding items only. Refresh whenever something ships (move the row from here to `_prep.md`'s shipped list). Useful when you want the short version of "what's still to do". |
