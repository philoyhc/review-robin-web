...
| S12 | Visibility-window axis — `instrument_view_policies.visible_when` (String(16), NULL) + `sessions.responses_release_stopped_at` (DateTime tz, NULL) | §3.3 + §3.4 | ✘ — additive prep needed | Two columns added in one Alembic file ahead of W15 (Band 3 editor) + W7 (resolver) + the new operator buttons. `visible_when` carries the per-(instrument, audience) window pick: `while_ongoing` (`[activated_at, deadline)`) / `after_release` (`[responses_release_at, +release_until_offset)`) / `throughout` (union of the two) / `always` (reserved for operator, who isn't an audience row). `responses_release_stopped_at` is stamped by the session-level **Stop release** button; the **Release responses now** button writes `responses_release_at = now()` and clears any prior stopped stamp (existing column, no schema change). View-time predicate: *release window is open* ⇔ `responses_release_at IS NOT NULL` AND `now() ≥ responses_release_at` AND `now() < responses_release_at + release_until_offset` AND `responses_release_stopped_at IS NULL`. Archive forces zero visibility for non-operator audiences at view time — no schema change. Both columns nullable, no backfill; lands inert. |
