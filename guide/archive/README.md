...
| `extract_data.md` | 522 | Extract data — Session Home card split + new Operations tab. Renamed the Session Home **Extract data** card to **Extract setup** (porting-shaped CSVs only + a four-CSV `{code}_setup.zip`) and added a new **Extract data** Operations tab for fine-grained response-data shaping: 2-column grid of half-width lens cards (by instrument; reviewer-metadata; reviewee-metadata) plus a full-width **Data shaper** (saved `data_shapes`, two-row chip layout, per-axis identification + aggregate chips, fan-out for `List items` / `Discrete steps`). Shipped 2026-05-29 → 2026-05-30 across PRs #1565 → #1627: UI buildout (~30 PRs), 6-PR wiring slice (DB schema → service layer → CRUD routes → file-gen + download route → Zip-all integration → Settings CSV round-trip), 4-PR bug-fix tail. Plan archived 2026-05-30; functional spec at `spec/extract_data.md`. |
