...
| `observers_mvp_planning.md` | 225 | The Design reframe + MVP definition + Implementation path + Implications carved out of `guide/observers.md` once the MVP shipped 2026-06-02. The current operating reference (storage shape, token design, where the wiring stands, cohort-consumer routes) stayed in `guide/observers.md` until that file itself archived 2026-06-03 — see the row below. |
| `observers.md` | ~290 | Standing notes for the observer participant role — storage shape (`observers.cohort_rule` JSON), token design decisions, the "where the wiring stands" + consumer + operator route list. Archived 2026-06-03 once the observer ladder closed (`observers_clean_up.md` items 13-16 all shipped). The shipped behaviour is captured in `spec/participant_model.md` + `spec/setup_pages.md` + `spec/csv_contracts.md` + `spec/extract_data.md`; this file is the design-decision record. |
| `observers_clean_up.md` | ~200 | Originally `guide/clean_up.md` — the standing punch-list of small-to-medium follow-ups identified by code review on the observer cohort editor PRs. Renamed + archived 2026-06-03 once every item closed and the file's content turned out to be entirely observer-specific. Items 1-12 came out of the initial code review on the cohort editor + cohort_rule persistence ladder (#1787 → #1796); items 13-16 were the post-MVP deferrals from the collation surface MVP. All sixteen struck through with their closing PR / commit. |
...
| `spec_sweep_18Aug.md` | ~140 | The 2026-08-18 whole-`spec/` drift audit (8 parallel passes over 34 files). §A unfinished-work items + §B currency debt all resolved in 18R Item 3 + the functional-spec/architecture revision; §C minor notes; §D post-sweep additions. Renamed from `guide/spec_sweep.md` + archived 2026-08-19. |
| `docs_sweep_19Aug.md` | ~330 | The 2026-08-19 whole-`docs/` audit (4 parallel passes over 20 files), sorted into revise-into-spec / update-in-place / consolidate / retire buckets with file:line evidence + a reference-repointing appendix. **All four buckets executed under Segment 19** (docs/ went 20 → 16 files); archived once complete. |
| `landing_pages.md` | ~200 | Standing notes for the Segment 18R Item 6 **landing-pages** work — audit of the entry routes (`/` served JSON, `/operator` 404'd, no role-aware routing), the gaps, the decision (role-aware redirect over a home hub), and the two-slice build log. Shipped (PRs #1982 / #1983) + archived 2026-08-19; current behaviour is in `spec/operator_ui_concept.md` "Entry & landing" + the reviewer-chrome specs. |
| `deferred_until_pilot_feedback.md` | 811 | Pilot-feedback-gated deferral ledger. Consolidated into `guide/deferred_consolidated.md` Part A + archived 2026-08-19; content preserved verbatim there. |
| `deferred_infra.md` | 114 | Infrastructure / platform-hardening deferral ledger (Azure portal + Postgres-only migrations). Consolidated into `guide/deferred_consolidated.md` Part B + archived 2026-08-19; content preserved verbatim there. |
| `future_possibilities.md` | 135 | Off-roadmap aspirational directions (AG Grid, randomizer/grouper). Consolidated into `guide/deferred_consolidated.md` Part C + archived 2026-08-19; content preserved verbatim there. |
| `codebase_assessment_09may.md` | 373 | Codebase-vs-functional-spec snapshot, 2026-05-09. Superseded by later assessments; the latest active snapshot lives at `guide/codebase_assessment_01jun.md`. |
...
| `codebase_assessment_30may.md` | 375 | Codebase-vs-functional-spec snapshot, 2026-05-30 (the close of Segment 18N housekeeping + Extract data slice Phases 1-3 + Self-review consolidation; 129 PRs in 3 days). 2,206 tests; ~46.2k production LOC; biggest file `assignments.py` at ~1,200 LOC after the 18N splits. Archived 2026-06-01 once superseded by `guide/codebase_assessment_01jun.md`. |
| `codebase_assessment_01jun.md` | 478 | Codebase-vs-functional-spec snapshot, 2026-06-01 (the close of the **participant-model surface arc** — URL remodel, observer roster + per-session toggles + Band 3 visibility editor + schedule authoring + reviewee `/results` body across all three modes + Acknowledge gesture + Observers Quick Setup / Extract + Validate-warning + L1 cleanup; ≈90 PRs in 2 days). 2,417 tests; ~50.3k production LOC. Observer collation surface deliberately paused. Appendix §9 carried file-split proposals for `assignments.py` / `scheduled_events.py` / `session_config_io/_apply.py` (all three still unactioned post-`03jun`). Archived 2026-06-03 once superseded by `guide/codebase_assessment_03jun.md`. |
| `codebase_assessment_03jun.md` | ~440 | Codebase-vs-functional-spec snapshot, 2026-06-03 (the close of the observer-collation MVP + cohort editor). Superseded by `codebase_assessment_17aug.md`; archived alongside it 2026-08-19. |
| `codebase_assessment_17aug.md` (+ `.json`) | ~390 | Codebase-vs-functional-spec snapshot, 2026-08-17 (the ~2.5-month window since 18O closure: June docs/identity sweep, 18P rehydrate, 18Q blob deferral, 18R save/lock, 18S Item 1 three-tier roles). 2,650 tests; 55,165 production LOC; watchlisted `_instruments.py` at 1,317. Retired to archive 2026-08-19 once superseded by `guide/codebase_assessment_19aug.md`. First snapshot to ship a JSON delta sidecar. |
...
| `url_remodel.md` | 260 | URL remodel — `/reviewer/` → `/me/` aggressive hard rename. Two-PR slice that landed 2026-05-30. **PR A (#1668)** moved the existing `/me` JSON + `/me/debug` debug endpoints to `/auth/me` + `/auth/me/debug`, freeing the namespace (12-file change). **PR B (this archive close-out)** flipped the four ``routes_reviewer/`` router prefixes from ``/reviewer`` to ``/me`` and bulk-renamed every callsite — ~340 occurrences across production code (URL builders, redirect targets, ``return_to.py`` regexes, ``breadcrumbs.py``, the invite-URL regex in ``invitations.py``), 16 templates, ~290 test ``client.get/.post`` strings, plus all 10 spec docs referencing `/reviewer/*` URLs. Folder name ``app/web/routes_reviewer/`` deliberately stays (optional polish — rename to ``routes_participant/`` later if/when the participant arc opens). No compatibility shim — single-author beta with no real-user URLs in flight; the email outbox is inert (Segment 14B not shipped) so any queued rows will regenerate with the new URL when 14B activates. Closed Q4 of the participant-model URL design ahead of 14B Part A. |
...
| `participant_model_upgrade.md` | 1133 | Participant-model umbrella design doc for the post-MVP arc — the planned evolution into a generalized participant system where reviewees and observers are first-class participants. Carried the §§1–12 design rationale + Appendix A's S / P / W identifier glossary with per-item status + PR pointers. Archived 2026-06-01 once the participant-model surface work had landed across PRs #1671 → #1761; the residual tail moved to `guide/segment_14B_email_infrastructure.md`'s appendix (W20 / W21 + magic-link schema) and `guide/observers.md` (W5 / W17). The shipped behavior is captured in `spec/participant_model.md` going forward. |
| `participant_model_remainder.md` | 29 | The participant-model outstanding-items filter that paired with `participant_model_upgrade.md`. Archived 2026-06-01 in the same sweep — every row had been moved to its home (W11 closed; W12 / W13 / W16 / W19 / W8 / L1 / L2 shipped; W5 / W17 → `observers.md`; W20 / W21 → `segment_14B_email_infrastructure.md` appendix). |
| `segment_18O_post_participants_model_file_splits.md` | 283 | Segment 18O — Post-participants-model file splits. Four-track housekeeping pass on production files in the 1,300+ LOC band before Segment 14B adds route-heavy bulk on top. **Track A (#1823)** carved `scheduled_events.py` (1,380 LOC) into a 7-module package (`_shared` / `_duration` / `_activation` / `_invites` / `_reminders` / `_release` + `__init__` orchestrator). **Track B (#1824)** carved `assignments.py` (1,426 LOC) into a 4-module package (`_shared` / `_coverage` / `_self_review` / `_generate`); biggest file in the repo drops to `assignments/_generate.py` ~764 LOC post-Track-B. **Track D (#1825)** carved `routes_reviewer/_surface.py` (1,299 LOC) into a 4-module package (`_status` / `_group_collapse` / `_context` / `_routes`). **Track C (#1826)** carved `session_config_io/_apply.py` (1,361 LOC) into eight per-section modules + a slim orchestrator. ~5,500 LOC redistributed across ~23 small modules; no behaviour change; 2,546 tests stayed green throughout. Shipped 2026-06-02 → 2026-06-03; archived 2026-06-16. |
| `segment_18P_patching_roundtrip.md` | 530 | Segment 18P — Patching the round-trip (harmonize + rehydrate). **Group 1** (PRs #1864 → #1870) closed the export→import / clone leaks in `spec/roundtrip_coverage.md` — feature toggles, instrument view policies, reviewer / reviewee / observer roster `status`, observer cohort rules, scheduling / retention anchors, data shapes, session tags, `band1_touched_links`. **Group 2** (scaffold #1872, polish #1873, F / G1 / G2 / G3 #1877 / H #1878) shipped **Rehydrate** — rebuild a live draft session from a complete extract set via the lobby button + `/operator/sessions/rehydrate` (Validate → Rehydrate), a pre-flight analyzer, an operator-scoped stash (`rehydrate_stashes`), a sectioned-`responses.csv` importer, and the `session_rehydrate.rehydrate_session` orchestrator; `docs/rehydrate.md` flipped "proposed" → "shipped". Shipped 2026-06-05; archived 2026-08-16. |
| `weaknesses_and_bugs_found_by_codex.md` | 161 | Prioritised assessment + action plan for the 2026-06-05 Codex weaknesses sweep. P0 (identity case-normalization) shipped as PRs #1836 → #1839 (Slices A + B + C — case-insensitive lookups across `get_or_create_user` + per-row reviewer / reviewee / observer CRUD — plus the historical-duplicates `.order_by(User.id).limit(1)` safeguard). P1 (Segment 14B email send) sequenced under its own segment plan. P2 (pilot polish) under Segment 20. P3 (large templates) and the storage-level Slice D follow-up lifted into `guide/deferred_until_pilot_feedback.md` with explicit lift triggers + pre-work audit query. P4 (stale comments) closed as standing policy. Archived 2026-06-28 once every actionable item had either shipped, deferred to the pilot-feedback ledger, or closed as policy. |
