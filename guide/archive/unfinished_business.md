...
### 24. Operator-editable email template editor · [feature] · medium · ✅ shipped 2026-05-05

**Closed by Segment 11E.** Editor at `/operator/sessions/{id}/setupinvite` ships per-template (Invitation / Reminder) override of subject + body + CC + BCC; merge fields render via `string.Template.safe_substitute` over the canonical five-tag set (`$reviewer_name` / `$session_name` / `$deadline` / `$help_contact` / `$invite_url`). Per-session `help_contact` column on `sessions` carries the operational help contact and surfaces on the reviewer surface as `Questions? Contact <X>`. PRs **#461 (PR 1) → #462 (PR 2-A) → #463 (PR 4) → #464 (PR 5) → #465 (PR 2)**, plus polish **#468**. The transport-side scaffolding (`EmailTransport` Protocol + `SmtpEmailTransport` + `GraphEmailTransport` typed stub + per-operator Settings page at `/operator/settings`) shipped in the same segment; the production send-activation work picks up in Segment 11C Part 2. Plan: `guide/archive/segment_11E_email_template_editor.md`. Broader transport landscape: `spec/email_infra_options.md` (Options A–D).

(Original entry preserved below for archaeology.)
