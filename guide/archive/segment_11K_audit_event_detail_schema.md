...
| `session.deleted` | `snapshot` (no top-level identity) | `{"session_id": …, "code": …, "name": …}` written with the FK column `session_id=NULL` | `{"snapshot": {"id": 17, "code": "CS101", "name": "Final Review"}}` — top-level identity slots are absent because the FK is null; identity lives inside `snapshot` on column-mirror keys (`id` / `code`). The audit-export consumer reads `detail.snapshot.id` for deletion events. | `sessions.py:171-179` |
