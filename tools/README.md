...
**Turn splits only where the stamp exists.** A slice whose first commit
...
The stamp is matched on its own line **anywhere** in the message, not as
a git trailer: git parses only the final paragraph as trailers, and 21 of
the first 38 stamps sat in a paragraph of their own and were silently
dropped that way (19R Item 7). Fix commits answering a reader or CI do
not carry the stamp and are not read for it.
