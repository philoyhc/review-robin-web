...
| `bench_roster_scale.py` | Roster-scale benchmark (**writes**) — seeds a synthetic session of a given size into a *local* Postgres, then times every operator page with the SQL / Python split and the per-request query count. The method behind `guide/app_responsiveness.md`. **Needs a Postgres cluster you are willing to write to**, which the suite's in-memory SQLite deliberately is not, so it is not in CI and not a repo dependency; `seed` refuses any non-loopback `DATABASE_URL`. Subcommands: `seed` / `seed-lobby` / `pin-rule` / `post` / `bench` (with `--only` to filter the page list) / `profile` — `pin-rule` + `post --path /workflow/prepare` reproduce the post-Generate state, which is the one that hurts. Wall-clock is machine-specific; the query counts and the SQL / Python split are the comparable figures. | `python3 tools/bench_roster_scale.py seed` |
...
**A renamed plan keeps its window.** A segment renumbered (`01` → `01a`)
answers the lookup under its *new* name with the commit that created that
name — the rename, or the archive move — so the window collapses onto it
and every honoured path reports as untouched. When a plan has an earlier
name, every name is therefore asked and the earliest answer wins; when it
has none, which is almost always, nothing changes. Earlier names come from
`--follow` and are filtered to plan names, because `--follow` matches on
similarity and will name a file that was never this plan.
