...
| Activating SMTP / Graph / ACS / third-party transactional sends out of the dev outbox; institutional Microsoft 365 tenants typically block basic SMTP AUTH, so the realistic production path is one of Options B–D in `spec/email_infra_options.md` | **Segment 11C Part 2** (consumes the 11E `EmailTransport` interface from Manage Invitations) plus subsequent backend implementations driven by the operator's IT conversation |
