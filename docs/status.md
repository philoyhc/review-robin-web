...
| `GET /operator/sessions/{id}` | Session Home (post-11B shape) — two-column body: left column stacks **Next Action card** (constant H2, blue border, fixed min-height, state-conditional Primary + Secondary buttons at the bottom; surfaces Validate Setup / Activate Session / Pause Session per lifecycle state), then Quick Setup and Extract Data placeholder cards via the canonical `placeholder_card` macro; right column stacks Session Details (with Edit link) and Danger Zone (Delete Data + Delete Session, both visible-but-disabled in `ready`). `?validated=1` re-runs setup validation and (when no blocking errors) marks the session `validated`; the Next Action card switches its primary button to **Activate Session** and surfaces the optional `acknowledge_warnings` checkbox. Spec at `spec/session_home.md`. |
