# Review Robin Web (RRW) — Quickstart — RETIRED 2026-09-06

> **Superseded by the in-app Guide at `/guide`.** Segment 19E rung 2 moved this
> material into the app and made that page canonical, so a reader finds the
> documentation where the app is rather than in a repository nobody using the
> app can see. Kept here rather than deleted: this file is the record of what
> the Guide was built from, and a retired file is a better answer to a stale
> link than a missing one.
>
> **Do not edit this file.** Corrections go to
> `app/web/templates/guide.html`. Anything below may already be out of date.
>
> One thing that did not survive the move: the nineteen `📷 Screenshot —`
