...
## Appendix — Quick setup commands (Ubuntu / WSL2)

Copy-paste-friendly commands for the workstation-side steps
that aren't already covered in `docs/cli_setup.md` Appendix A.
Uses the PostgreSQL apt repo to pin the client at version 16
(matching the Flexible Server), and the `gh` credential helper
so `git push` never prompts for a password.

### Installing PostgreSQL client 16

Ubuntu's default `postgresql-client` package trails the current
Postgres major version — use PostgreSQL's official apt repo to
get client 16, which is what the Flexible Server speaks.
...
### Logging into GitHub + configuring git identity
...
Then `git log -1 origin/main --pretty=format:"%an <%ae>"` (or
any log inspection) will show the configured identity was used
for the ephemeral commit. If `gh auth setup-git` didn't run or
the identity isn't set, `git commit` will error with
`Please tell me who you are` or `git push` will 403.
