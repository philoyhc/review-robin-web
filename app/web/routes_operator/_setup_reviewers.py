    ...
    *,
    ...
    return _templates.TemplateResponse(
        ...
        "operator/session_reviewers.html",
        {
            ...
            # The add row's own id, and what `Add new` lands on. It
            # named an editor CARD until that card was retired for
            # holding nothing but a heading; the editor is the row now,
            # so this is an id ON the row. `Edit` does not use it — it
            # builds `#reviewer-row-<id>` from the id it already has.
...
@router.post("/sessions/{session_id}/reviewers/bulk-delete")
def reviewers_bulk_delete(
    reviewer_ids: list[int] = Form(default=[]),
    confirm: str | None = Form(default=None),
    acknowledge_response_loss: str | None = Form(default=None),
    filter_status: str = Form(default="all"),
