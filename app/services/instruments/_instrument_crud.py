...
instrument seeding helpers, the Band 1 group / unit-of-review
plumbing, the per-instrument column-widths writer, and the
session-level bulk accepting / visibility toggles.

Originally Slice 4 of the §12.A ladder
(``guide/archive/major_refactor.md``) — the final slice of the
2026-05-09 package split. Segment 18N PR 2 then carved this
file's two largest concentrations out into sibling slices:
``_band2.py`` owns the Band 2 state save +
``_sync_response_fields_to_db`` + ``_sync_display_field_visibility``,
and ``_pagination.py`` owns the Segment 18M reorder +
per-instrument page-break helpers. What stays here is the basic
"what is an instrument and how does the operator manipulate it"
core.

Saves emit ``instrument.created`` / ``.deleted`` /
``.description_updated`` / ``.short_label_updated`` /
``instrument.group_kind_updated`` / ``instrument.column_widths_updated``
