...
# Cross-slice state (sliced in PR 0). ``_SAVED_STATE_EVENT_TYPES``
# stays private to ``_state.py`` — only ``saved_state_for_session``
...
#
# ``_instrument_label`` is private but exercised by
# ``tests/integration/test_segment_11l_short_label.py``; preserve it
# in the re-export wall so the test surface stays byte-identical.
from ._legacy import (
