...
The practice has a name now. Through 2026 the term **spec-driven development** (SDD) went mainstream for the family of methods in which specifications are the durable source of truth and code is the thing kept in agreement with them. RRW was doing a form of this before the term arrived, and — more usefully for a reader — it is doing a *particular* form of it, with rules about which direction authority runs and when, that the general term does not fix. This document states that form, records why each of its parts is the way it is, and says plainly where it departs from what SDD prescribes and where it falls short of it.
...
- **Consequential UI lands scaffold-first** (convention dated 2026-08-15). A new page, card or navigation affordance lands as a static placeholder — real copy and layout, inert controls — in its own PR before any behaviour is wired. The surface is agreed on the placeholder; the wiring PRs then carry no UI churn. It is the UI-shaped version of writing the contract before the implementation.
...
**Trade-off, stated plainly.** There is a window of spec drift inside every open segment, by design. During it, `spec/` describes the last settled state and `guide/` describes the intended next one; a reader who consults only `spec/` mid-segment will be wrong about what is being built. The mitigation is that segments are short (days, not months), the plan names its doc impact up front, and the definition of done will not close without the spec edit. The cost that mitigation does not cover is Section 6.5's: drift that is *not* noticed at the close survives it.
...
**Trade-off.** The reader works when run, and "when run" is the honest qualifier. Both agents are invoked on request, not by a hook; six commit messages name `spec-writer` and five name `diff-reviewer`. The diff reviewer has already earned its place — its first run, on the audit's own PRs, found five claim-versus-reality defects, fixed in `5d017eb9`, including a commit message that claimed three milestones came from a file that contained none of them — but a reader that is not run is a paragraph.
...
| Maker and checker separate | `spec-writer` (writes spec to match code) + `diff-reviewer` (reads diff cold against spec; report-only; no model pin) | `.claude/agents/`; 6 + 5 commit messages naming them |
