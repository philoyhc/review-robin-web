...
def test_every_unlock_control_is_inert(client, db):
    ...
    undisabled = [c for c in controls if "disabled" not in c]
...
def test_the_three_cards_it_will_absorb_are_still_live(client, db):
    ...
    html = _page(client, _session_with_reviewers(client, db, "scaf4"))
    assert 'id="upload-csv"' in html
    assert "danger-zone" in html
    # The friendly-labels editor still renders its own card.
    assert "field-labels-editor" in html or "Friendly labels" in html
