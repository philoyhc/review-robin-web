...
def test_the_setup_pager_links_navigate(client: TestClient, db: Session) -> None:
    ...
    # The ``#reviewers-table-card`` tail is 19J.8's landing fragment; the
    # offsets it rides on are exactly the ones this test has always
    # asserted. ``tests/integration/test_pager_anchor.py`` owns the
    # fragment itself.
    assert f'href="{base}200#reviewers-table-card"' in body
    assert f'href="{base}400#reviewers-table-card"' in body
