...
The tag line was originally built solely from the boundary tag
values (the values that defined group membership). That silently
dropped any non-boundary reviewee tag the operator selected as a
display field — the operator's Band 2 preview showed every
selected tag pill's value, but the reviewer surface didn't. This
test pins the contract: the tag line includes the values of every
visible ``reviewee.tag_*`` display field on the instrument,
comma-joined, in display order. Falls back to the boundary-key
composition when there are zero reviewee.tag_* display fields (so
the line is never blank when a boundary tag value exists).
"""
